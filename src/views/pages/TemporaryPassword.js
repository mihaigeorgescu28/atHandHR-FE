...
function TemporaryPassword() {
  ...
  const [noTemporaryPassswordError, setNoTemporaryPasswordError] = useState(false);
  ...
  //const uidIndex = urlParts.indexOf('temporaryPassword') + 1; // Assuming UID is right after 'temporaryPassword'
  ...
  useEffect(() => {
   ...
   // useEffect hook to check for temporary password when the page loads
   useEffect(() => {
    const checkForTemporaryPassword = async () => {
      try {
          // Extract UID from the URL
      const urlParts = window.location.pathname.split('/');
      ...
      setUserId(userId)
        ...
        if (response.data.success) {
          ...
          setNoTemporaryPasswordError(false);
        } else {
          console.log('User with temporary password not found.');
          setNoTemporaryPasswordError(true);
        }
      } catch (error) {
  ...
  { noTemporaryPassswordError ? (
    // Render a simple error icon if noTemporaryPasswordError is true
    <div className="error-icon" style={{ textAlign: 'center' }}>
  <FontAwesomeIcon icon={faExclamationCircle} size="5x" style={{ color: 'red', marginBottom: '10px' }} />
  <p style={{ fontWeight: 'bold' }}>Invalid URL. Please contact your administrator!</p>
</div>


  ) : (
    // Otherwise, show the existing form
    <>
      <div className="password-icon">
        <FontAwesomeIcon icon={faLock} size="5x" style={{ color: 'gray' }} />
      </div>
      <br />
      ...
      {!success && <p style={{ color: "red" }}>{passwordValidationError}</p>}
      ...
      {!success && <p style={{ color: "red" }}>{passwordMatch}</p>}
